...
top_percent = .05; % what proportion of top performers will be averaged to create "optimal" connectome
...
load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/data/DS1_data/gradCPT_flatmats_motion_behav_data']);
load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS1_StabTypDisc']);
...
[r_absdiff, p_absdiff] = corr(behav_ATTN, abs_diff_perf, 'rows','pairwise');
...
elseif ds == 2
load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS2_StabTypDisc']);
 load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/data/DS2_data/Chun_flatmats_motion_behav']);
 for m = 1:size(mot,2)
avg_mot_pre(:,m) = mot{m};
end
...
abs_diff_perf = abs(behav_ATTN_run1 - behav_ATTN_run2);
[r_absdiff, p_absdiff] = corr(behav_ATTN, abs_diff_perf, 'rows','pairwise');

abs_diff_perfwm = abs(behav_WM_run1 - behav_WM_run2);
[r_absdiff_wm, p_absdiff_wm] = corr(behav_WM, abs_diff_perfwm, 'rows','pairwise');
...
load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS3_StabTypDisc']);
 load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/data/DS3_data/HCP_flatmats_motion_behav_data']);
%  load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS3_StabTypDisc_meanREST.mat']);
...
save(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS3_RestTyps.mat'],'OverRest_typ_ATTN', 'sig_OverRest_typ_ATTN' ,'OverRest_typ_WM', 'sig_OverRest_typ_WM');
...
[rOD_WM,pOD_WM] = corr(Opt_WM{3,4},Disc{3,4},'rows','pairwise','type','Spearman');
 ...
 noOut_log = (Disc{1,2} < (nanmean(Disc{1,2}+(2.5*nanstd(Disc{1,2})))));
 Disc_noOut = Disc{1,2};
 Disc_noOut(~noOut_log) = NaN;
         [corr_disc_ATTN_noOut, sig_disc_ATTN_noOut] = partialcorr(Disc_noOut, behav_ATTN, [nanmean([mot{1}, mot{2}],2), abs(mot{1}-mot{2})],'rows','pairwise','type','Spearman');     


 noOutWM_log = (Disc{3,4} < (nanmean(Disc{3,4}+(2.5*nanstd(Disc{3,4})))));
 DiscWM_noOut = Disc{3,4};
 DiscWM_noOut(~noOutWM_log) = NaN;
[corr_disc_WM_noOut, sig_disc_WM_noOut] = partialcorr(DiscWM_noOut, behav_WM, [nanmean([mot{3}, mot{4}],2), abs(mot{3}-mot{4})],'rows','pairwise','type','Spearman');     


end

load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS1_StabTypDisc']);
MIN_Disc1 = min(min([Disc{1,2}, Disc{1,3}, Disc{2,3}]));
MAX_Disc1 = max(max([Disc{1,2}, Disc{1,3}, Disc{2,3}]));


load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS2_StabTypDisc']);
MIN_Disc2 = min(min([Disc{1,2}, Disc{3,4}]));
MAX_Disc2 = max(max([Disc{1,2}, Disc{3,4}]));

load(['/Users/annacorriveau/Documents/GitHub/FCStability_Typicality/outputs/DS3_StabTypDisc']);

MIN_Disc3 = min(min([Disc{1,2}, Disc{3,4}]));
MAX_Disc3 = max(max([Disc{1,2}, Disc{3,4}]));
